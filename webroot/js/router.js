...
const router_state = {
	...
	// last profile applied via Tools (name + the cc/qdisc it set), so a bufferbloat
	// run can tag its Results record with the profile — but only while still active.
	activeProfile: null,
};
...
document.addEventListener('DOMContentLoaded', async () => {
	...
	function loadPage(pageName) {
		return new Promise(async (resolve, reject) => {
			...
			currentPage.style.transition = 'opacity 0.12s ease';
			...
			}, 120); // Wait for exit animation to finish
		});
	}
